...
class FirestorePrintBridge:
    ...
    def _on_snapshot(self, collection_snapshot: Any, changes: list[Any], read_time: Any) -> None:
        ...
        for order_id, items in grouped.items():
            ...
            try:
                self.logger(f"Cargando ticket comanda #{order_id} (Mesa {table_label}, {len(items)} item(s))...", "MATCH")
                ticket = self._build_ticket(order_id, items)
                self.logger(f"Enviando comanda #{order_id} a la impresora '{self.config.printer_name}'...", "INFO")
                PrinterService.print_bytes(self.config.printer_name, ticket)
                for item in items:
                    self.registry.mark(item.get("id") or item.get("doc_id", ""))
                self.logger(f"✓ IMPRESA comanda #{order_id} | Mesa {table_label} | {len(items)} item(s)", "PRINT")
    ...
    def _on_print_jobs_snapshot(self, collection_snapshot: Any, changes: list[Any], read_time: Any) -> None:
        ...
        for change in changes:
            ...
            try:
                ...
                self.logger(f"✓ IMPRESA Nota de Consumo #{job_id} | Cliente: {client}", "PRINT")
            except Exception as exc:  # noqa: BLE001
...
class PrinterBridgeApp:
    ...
    def _build_ui(self) -> None:
        ...
        self.btn_stop = ttk.Button(buttons_frame, text="⏹ Detener", command=self._stop_listener)
    ...
    def _start_listener(self) -> None:
        ...
        try:
            ...
            self.bridge.start()
            self.status_var.set("● Listener: ACTIVO (Escuchando Firestore)")
            self.status_lbl.configure(foreground="#16a34a")
            self._push_event("Listener activado exitosamente", "INFO")
        except Exception as exc:  # noqa: BLE001
